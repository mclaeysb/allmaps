<script lang="ts">
  ...
  import { computeBbox, mergePartialOptions } from '@allmaps/stdlib'
  ...
  export type GeoreferencedMapsComponentOptions =
  ...
  let stateOptions: Partial<GeoreferencedMapsComponentOptions> = $state({})
  ...
  let mergedOptions = $derived(mergePartialOptions(stateOptions, options))
  ...
  onMount(() => {
    ...
    if (mergedOptions.addNavigationControl) {
      map.addControl(new maplibregl.NavigationControl(), 'top-left')
    }
    if (mergedOptions.addGeolocateControl) {
      map.addControl(
  ...
  $effect(() => {
    ...
    if (mapOrImage === 'map') {
      ...
      if (selectedGeoreferencedMap && selectedMapId) {
        ...
        warpedMapLayer?.setMapsTransformationType(mapIds, 'helmert')
      }
      stateOptions = { transformationType: 'helmert', applyMask: false }
    }
  })
  ...
  $effect(() => {
    ...
    warpedMapLayer.setOptions(mergedOptions)
