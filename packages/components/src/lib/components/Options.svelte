<script lang="ts">
  ...
  import type { TransformationTypeAndDefault } from './TransformationTypePicker.svelte'
  ...
  let opacity = $state(1)
  let transformationType = $state(
    'fromAnnotation' as TransformationTypeAndDefault
  )
  let internalProjection = $state(undefined)

  let nextTransformationType = $state(
    'fromAnnotation' as TransformationTypeAndDefault
  )
  ...
  $effect(() => {
    ...
    if (transformationType !== 'fromAnnotation') {
      options = { ...options, transformationType }
    }
    if (internalProjection !== undefined) {
      options = { ...options, internalProjection }
    }
  })
</script>
...
<div class="grid grid-cols-2 gap-4">
  ...
  <TransformationTypePicker
    bind:selectedTransformationType={transformationType}
    bind:nextTransformationType
  ></TransformationTypePicker>
  <div class="text-sm content-center">Projection</div>
  <ProjectionPicker
    {projections}
    bind:selectedProjection={internalProjection}
